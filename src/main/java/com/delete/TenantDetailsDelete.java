...
public class TenantDetailsDelete extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		out.print("<!DOCTYPE html>"
				...
				+ "    <title>Tenant Details Delete</title>"
				...
				+ "        <form id='tenant' name='tenant' method='post' action='TenantDetailsDeleteAction'>");
		
		for(TenantDetailsBean t: tenantDetailsFetch) {
			
			out.print("			   <input type='hidden' id='tId' name='tId' value='"+t.getTId()+"'>"
					+ "            <label>Owner Name</label>"
					+ "            <input type='text' id='tName' name='tName' readonly='true' value='"+t.getTName()+"'>"
					+ "            <br><br><br>"
					+ "            <div class='textarea'>"
					+ "              <label>Address</label>"
					+ "              <textarea id='address' name='address' readonly='true' rows='5' cols='60'>"+t.getTAddress()+"</textarea>"
					+ "            </div>"
					+ "            <br><br><br>"
					+ "            <label>Contact No.</label>"
					+ "            <input type='tel' id='contact' name='contact' readonly='true' value='"+t.getTContact()+"' pattern='[6-9]{1}[0-9]{9}'>"
					+ "            <br><br><br><br>"
