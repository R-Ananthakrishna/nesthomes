...
public class MonthlyGasUsageDelete extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException{
		...
		ArrayList<MonthlyGasUsageBean> monthlyGasUsageFetch=null;
		
		try {
			
			monthlyGasUsageFetch = MonthlyGasUsageDAO.fetchMonthlyGasUsageById(mgcId);
		...
		out.print("<!DOCTYPE html>"
				...
				+ "    <title>Monthly Gas Usage Delete</title>"
				...
				+ "        <form id='gasUsage' name='gasUsage' method='post' action='MonthlyGasUsageDeleteAction'>");
		
		for(MonthlyGasUsageBean mgc: monthlyGasUsageFetch) {
			
		
			out.print("    	   <input type='hidden' id='mgcId' name='mgcId' value='"+mgc.getMgcId()+"'>"
					+ "        <label>Flat</label>"
			...
			out.print("            </select>"
					+ "            <br><br><br>"
					+ "            <label>Unit</label>"
					+ "            <input type='number' id='unit' name='unit' readonly='true' value='"+mgc.getGasUnit()+"' min='0' step='0.01'>"
					+ "            <br><br><br><br>"
