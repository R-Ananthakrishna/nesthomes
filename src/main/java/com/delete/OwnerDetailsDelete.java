...
public class OwnerDetailsDelete extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		ArrayList<OwnerDetailsBean> ownerDetailsFetch=null;
		
		try {
			
			ownerDetailsFetch = OwnerDetailsDAO.fetchOwnerById(owId);
		...
		out.print("<!DOCTYPE html>"
				...
				+ "    <title>Owner Details Delete</title>"
				...
				+ "        <form id='owner' name='owner' method='post' action='OwnerDetailsDeleteAction'>");
		
		for(OwnerDetailsBean ow: ownerDetailsFetch) {
			
			out.print("			   <input type='hidden' id='owId' name='owId' value='"+ow.getOwId()+"'>"
					+ "            <label>Owner Name</label>"
					+ "            <input type='text' id='oName' name='oName' readonly='true' value='"+ow.getOwName()+"'>"
					+ "            <br><br><br>"
					+ "            <div class='textarea'>"
					+ "              <label>Address</label>"
					+ "              <textarea id='address' name='address' readonly='true' rows='5' cols='60'>"+ow.getOwAddress()+"</textarea>"
					+ "            </div>"
					+ "            <br><br><br>"
					+ "            <label>Contact No.</label>"
					+ "            <input type='tel' id='contact' name='contact' readonly='true' value='"+ow.getOwContact()+"' pattern='[6-9]{1}[0-9]{9}'>"
					+ "            <br><br><br><br>"
