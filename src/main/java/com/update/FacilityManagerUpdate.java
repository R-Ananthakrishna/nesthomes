...
public class FacilityManagerUpdate extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		out.print("<!DOCTYPE html>"
				...
				+ "    <title>Facility Manager Update</title>"
				...
				+ "        <form id='fManager' name='fManager' method='post' action='FacilityManagerUpdateAction'>");
		
		for(FacilityManagerBean fm: facilityManagerFetch) {
			
			out.print("			   <input type='hidden' id='fmId' name='fmId' value='"+fm.getFmId()+"'>"
					+ "            <label>User Name</label>"
					+ "            <input type='text' id='uName' name='uName' value='"+fm.getFmName()+"'>"
					+ "            <br><br><br>"
					+"			   <label>Designation</label>"
					+ "            <input type='text' id='designation' name='designation' value='"+fm.getFmDesignation()+"'>"
					+ "            <br><br><br>"
					+ "            <div class='textarea'>"
					+ "              <label>Duties</label>"
					+ "              <textarea id='duties' name='duties' rows='5' cols='60'>"+fm.getFmDuties()+"</textarea>"
					+ "            </div>"
					+ "            <br><br><br>"
					+ "            <label>Contact No.</label>"
					+ "            <input type='tel' id='contact' name='contact' value='"+fm.getFmContact()+"' pattern='[6-9]{1}[0-9]{9}'>"
					+ "            <br><br><br><br>"
