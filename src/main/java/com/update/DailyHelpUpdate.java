...
public class DailyHelpUpdate extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException{
		...
		out.print("<!DOCTYPE html>"
				...
				+ "        <form id='dHelp' name='dHelp' method='post' action='DailyHelpUpdateAction'>");
		...
		for(DailyHelpBean dh:dailyHelpFetch) {
			...
			out.print("			   <input type='hidden' id='dhId' name='dhId' value='"+dh.getDhId()+"'>"
					+ "            <label>Name</label>"
					+ "            <input type='text' id='uName' name='uName' value='"+dh.getDhName()+"'>"
					+ "            <br><br><br>"
			...
			out.print("            </select>"
					...
					+ "            <div class='textarea'>"
					+ "              <label>Address</label>"
					+ "              <textarea id='address' name='address' rows='5' cols='60'>"+dh.getDhAddress()+"</textarea>"
					+ "            </div>"
					+ "            <br><br><br>"
					+ "            <label>Contact No.</label>"
					+ "            <input type='tel' id='contact' name='contact' value='"+dh.getDhContact()+"' pattern='[6-9]{1}[0-9]{9}'>"
					+ "            <br><br><br><br>"
